...
- **Are file associations and protocol handlers in `ld_settings` first scope?** No. They belong to the planned `ld_desktop` extraction.
- **Is autostart in `ld_settings` first scope?** No. The current implementation is temporary prototype evidence; stable autostart behavior belongs to `ld_desktop`.
