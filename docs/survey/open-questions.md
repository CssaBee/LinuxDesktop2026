...
- **Should file watching become the next implementation module?** Partly answered: the focused audit and library follow-up point toward a tiny `ld_watch` facade, but the ADR/API sketch still needs to lock the first public boundary.
...
- **Should `ld_watch` depend on libuv?** Current leaning: no required dependency in the first sample; keep libuv as the main reference and possible optional backend because its event loop is a large API choice.
...
- **How should overflow be tested?** Use a simulated backend or injected event path for first tests; do not rely on forcing a real kernel queue overflow in routine CI.
- **How much debounce belongs in core?** Include a small opt-in debounce/settle helper for ShareX-style workflows, but keep higher-level task routing application-owned.
