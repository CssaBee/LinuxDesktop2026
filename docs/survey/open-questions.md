...
- **Should file watching become the next implementation module?** Partly answered: the focused and broader audits point toward `ld_watch`, with ADR 0010 defining the first public boundary before prototype work.
...
- **What should happen before `ld_watch` code?** Shared C++ diagnostics now live in `ld_core`; next, finish the broader watcher audit, then use ADR 0010 as the API sketch before the prototype.
