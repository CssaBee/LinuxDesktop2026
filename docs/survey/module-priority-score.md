...
**Current watcher direction**: the broad `ld_watch` prototype now exists, with native Linux `inotify` as the default Ubuntu backend, native Windows `ReadDirectoryChangesW` implemented behind the backend seam, and optional libuv verified as a preferred backend when selected through CMake. Public C++ API stabilization has started with backend capability identity, named diagnostic-code constants, `watch_id` equality, timeout-capable pull delivery, optional settled-file timeout policy, and platform-neutral single-file `root_relative` semantics. Recommend libuv directly for apps that already own a libuv loop and only need coarse change/rename notifications; keep `ld_watch` for migration-shaped paths, diagnostics, recursive-policy honesty, and settled-file behavior. Windows watcher smoke coverage is wired into CI, Windows CI is green, and the first recursive/save-by-replace/remove-rename/resource-limit/settled-file hardening pass is implemented; keep native Windows verification green, postpone `ld_watch` C ABI design until release preview, and expand capability fields only when stress tests justify them.

**Thirteenth priority started**: expanded `ld_settings` settings/Registry survey. The expanded app and platform-equivalent audits showed real requirements for named roots, component roots, portable levels, config layers, migration plans, Windows Registry support, Linux equivalents for relevant Registry effects, autostart, and managed/enforced policy. ADR 0012 now narrows the `ld_settings` ship boundary and requires extraction: generic paths to `ld_paths`, desktop effects to `ld_desktop`, and migration behavior to `ld_migration`.

**Current `ld_settings` direction**: keep settings/config roots, layers, hydration, writes, and diagnostics; route generic root policy through `ld_paths`; extract Registry-equivalent desktop effects and migration behavior before ship-candidate status.
