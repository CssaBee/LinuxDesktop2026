...
- local watcher performance evidence now measures raw delivery throughput,
  queue depth, RSS growth, settled-file latency, pending settled work,
  large-tree coalescing, product validation-call counts, and queue-saturation
  drop reporting in `ld_watch_performance_probe`; it does not currently
  justify replacing `std::filesystem::path` in the watcher API,
