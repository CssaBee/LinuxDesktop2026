...
Cleanup reports are uninstall-shaped status rows, not raw deletion recipes.
Bundle plan/query/remove calls report generated artifact paths and explicit
cleanup rules with `cleanup_status` values such as `planned`, `removed`,
`already_absent`, `skipped_duplicate`, `manual_follow_up`, and `failed`.
Whole-file shared stores such as `mimeapps.list` are reported for manual
follow-up because ld_desktop owns generated entries inside the file, not the
file itself. Parent cleanup is limited to requested empty-parent removal and
reports whether the parent was left non-empty or removed.
...
- cleanup report query and remove for artifacts previously generated by
  `ld_desktop`.
