...
- Settled-file support now runs outside the raw backend delivery path, coalesces repeated events by source/path, schedules pending paths by deadline so one slow path cannot block ready paths, bounds distinct pending paths with `max_pending_paths`, reports `watch.queue.overflow` with rescan-required state when that capacity is exceeded, reports `watch.settle.timeout` when an optional timeout expires, cancels pending settled events when a watch is removed, and has larger-batch test coverage.
