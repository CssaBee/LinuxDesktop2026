...
bool flush_parent_directory(const std::filesystem::path& path, std::error_code& ec)
{
    ...
    return flushed;
#else
    const int handle = ::open(parent.c_str(), O_RDONLY | O_DIRECTORY);
    if (handle == -1) {
    ...
    std::error_code& ec)
{
    ...
    for (int attempt = 0; attempt != 128; ++attempt) {
        const auto candidate =
            parent / (stem + ".tmp." + std::to_string(current_process_id()) + "." + std::to_string(attempt));
    ...
    std::vector<diagnostic>& diagnostics)
{
    ...
    if (!wrote) {
        diagnostics.push_back(make_diagnostic(severity::error, "write-failed", "Could not write target content", target));
        if (!close_file_handle(handle)) {
            diagnostics.push_back(make_diagnostic(severity::error, "close-failed", system_error_code().message(), target));
        }
        return false;
    }
...
bool write_plain_file(const std::filesystem::path& target, const std::string& content)
{
...
{
    if (path.empty()) {
        diagnostics.push_back(make_diagnostic(severity::error, "empty-directory", "Cannot create an empty directory path"));
        return;
    }
    ...
    if (std::filesystem::exists(path, ec)) {
        if (!std::filesystem::is_directory(path, ec)) {
            diagnostics.push_back(make_diagnostic(severity::error, "path-not-directory", "Path exists but is not a directory", path));
        }
        return;
    }

    if (std::filesystem::create_directories(path, ec)) {
        diagnostics.push_back(make_diagnostic(severity::info, "directory-created", "Created directory", path));
        return;
    }

    if (ec) {
        diagnostics.push_back(make_diagnostic(severity::error, "directory-create-failed", ec.message(), path));
    }
}
...
{
    durable_file_write_report report;
    ...
    if (!options.atomic_replace && options.keep_backup && std::filesystem::exists(options.target, ec)) {
        ...
        if (ec) {
            report.diagnostics.push_back(make_diagnostic(severity::error, "backup-copy-failed", ec.message(), backup));
            return report;
        }
    ...
    if (options.atomic_replace) {
        ...
        if (!create_secure_temp_file(options.target, write_target, temp_handle, ec)) {
            report.diagnostics.push_back(make_diagnostic(severity::error, "temp-open-failed", ec.message(), options.target));
            return report;
        }
        report.temp_path = write_target;
        if (!write_all_bytes(temp_handle, options.content)) {
            report.diagnostics.push_back(
                make_diagnostic(severity::error, "temp-write-failed", "Could not write temporary target content", write_target));
            if (!close_file_handle(temp_handle)) {
                report.diagnostics.push_back(make_diagnostic(severity::error, "temp-close-failed", system_error_code().message(), write_target));
            }
        ...
        if (!write_plain_file(write_target, options.content)) {
            report.diagnostics.push_back(make_diagnostic(severity::error, "write-failed", "Could not write target content", write_target));
            return report;
        }
    ...
    if (validate) {
        ...
        if (!validate(write_target, validation_message)) {
            ...
            if (options.atomic_replace) {
                ...
                if (ec) {
                    report.diagnostics.push_back(make_diagnostic(severity::warning, "temp-cleanup-failed", ec.message(), write_target));
                } else {
                    report.diagnostics.push_back(
                        make_diagnostic(severity::info, "temp-cleaned", "Removed invalid temporary file", write_target));
                }
                ...
                if (ec) {
                    report.diagnostics.push_back(make_diagnostic(severity::error, "backup-restore-failed", ec.message(), options.target));
                } else {
                    report.diagnostics.push_back(
                        make_diagnostic(severity::warning, "backup-restored", "Restored previous target from backup", options.target));
                }
            }
            return report;
        }
    ...
    if (options.atomic_replace) {
        if (!replace_file(write_target, options.target, ec)) {
            report.diagnostics.push_back(make_diagnostic(severity::error, "atomic-replace-failed", ec.message(), options.target));
            std::filesystem::remove(write_target, ec);
            return report;
        }
        if (options.durable_write) {
            ...
            if (!flush_parent_directory(options.target, flush_ec)) {
                report.diagnostics.push_back(
                    make_diagnostic(severity::error, "parent-flush-failed", flush_ec.message(), options.target.parent_path()));
                return report;
            }
        }
    }
    ...
    const auto persisted_content = read_text_file(options.target, read_ec);
    if (read_ec || persisted_content != options.content) {
        ...
        if (report.backup_path) {
            ...
            if (restore_ec) {
                report.diagnostics.push_back(make_diagnostic(severity::error, "backup-restore-failed", restore_ec.message(), options.target));
            } else {
                report.diagnostics.push_back(make_diagnostic(
                    severity::warning,
