...
- [ ] FlavorTests identify public methods that currently return or store
  LinuxDesktop2026 report types where a product-native status, enum, boolean, or
  diagnostic summary would fit better.
- [ ] OBS keeps its C-shaped integer/buffer API boundary while still using
  LinuxDesktop2026 internally.
- [ ] Notepad++ and other C++ Flavor slices translate write/root/migration
  reports at product boundaries when doing so improves local reasoning.
- [ ] New write/root/migration convenience APIs are not treated as complete until
  their FlavorTest usage avoids unnecessary report leakage.
- [ ] Documentation states that full LinuxDesktop2026 report propagation is fine
